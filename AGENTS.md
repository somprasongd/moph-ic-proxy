...
- **Before every commit / tag**, update `CHANGELOG.md` (Keep a Changelog format) using `Added`/`Changed`/`Fixed`/`Removed`/`Security` sections:
  ...
  - On each release: bump `package.json` (`version` field) and move `[Unreleased]` entries under a new `[X.Y.Z] - <date>` heading (and update the compare links at the bottom), commit that as `chore(release): vX.Y.Z`, **then** create the git tag `vX.Y.Z` — the tag must point at the commit that contains the changelog update, not the one before it.
  - **Before tagging**: re-read `package.json` and confirm its `version` matches the `vX.Y.Z` you're about to tag. If it's still on the previous version, bump it in the same release commit before tagging.
...
- Tests live in `test/unit/*.test.js` and `test/integration/*.test.js` using Node's built-in `node:test` + `node:assert/strict` (no test framework dependency); `npm test` runs both. Each file runs in its own process and modules read env at `require` time — so every file sets the env it needs at the top, **before** requiring `src/` (e.g. point `MOPH_IC_AUTH`/`MOPH_IC_API` at a fake upstream). `test/helpers/fake-upstream.js` is an in-process HTTP server that mimics the MOPH upstreams (`/token`, `/api/echo`, 401-once `/api/stale`, gzip error, slow, 1MB binary) and records every request it receives. Integration tests boot the real app: `app.test.js` requires `src/index.js`'s exported `main()` in-process (returns the HTTP server), `shutdown.test.js` spawns `node src/index.js` as a child and asserts SIGTERM drains in-flight requests before exit 0. `API_KEY_FILE` overrides where `keygen` stores the API-key file so tests never touch `.authorized_key/.access.key`. The files in `test/*.http` are VS Code REST Client requests for exercising a running server (e.g. real MOPH/UAT upstreams), not automated tests.
- Production: `docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d` (uses `moph-api-proxy.env`, image from ghcr.io). CI (`.github/workflows/cicd.yml`) builds/pushes the image and redeploys the dev server via SSH on every push to `main`; pushing a `v*` tag additionally builds and pushes a version-tagged image (`:X.Y.Z`, from the tagged commit — the tag run never pushes `latest` and never redeploys). Rollback = pin the compose `image:` to an old version tag.
...
3. `src/middleware/use-auth.js` — when `USE_API_KEY=true`, requires `x-api-key` (header or query, verified against `.authorized_key/.access.key` via `src/helper/api-key.js`; file persists in a Docker volume in prod). The query `x-api-key` is stripped before proxying. `api-key.js` is a vendored, byte-compatible replacement for `uuid-apikey` + `encode32` (UUID ↔ Base32-Crockford with a 3-bit parity checksum); `test/fixtures/api-key-vectors.json` freezes known vectors from the original module as a compatibility lock — if you touch the algorithm, those tests must keep passing or every issued key stops verifying.
